...
const AccountantsNetworks = () => {
  ...
  useEffect(() => {
    getPartnersByEnvironmentalAccountants();
  }, []);
  ...
  async function getPartnersByEnvironmentalAccountants() {
    const apiUrl = `${process.env.REACT_APP_API_PORT}/partners/getPartnersByCategory`;
    try {
      // Make the GET request using Axios
      const response = await axios.post(apiUrl, {
        categoryName: "Environmental Accountants",
      });
  ...
  return (
    <StrictMode>
      <div className="business">
        <div className="sm:flex justify-center items-center p-10 gap-10 w-3/4 max-sm:w-full max-sm:p-2 max-sm:gap-4 bg-black/50">
          ...
          <div className="text-lg max-sm:text-sm">
            Environmental accountants play a pivotal role in making our economy
            more resilient and regenerative. By meticulously tracking and
            reporting on natural capital & ecosystem services, they provide
            crucial data used by governments, organizations, and individuals for
            informed decision-making. This results in better policies, business
            practices and impact investments that help reduce carbon emissions,
            conserve natural resources, and increase climate, economic and
      ...
      <div className="nwB">
        ...
        <div className="nwBb">
          At Impact Ecosystem, we're dedicated to holistic solutions,
          integrating non-profit grants, regenerative product discounts, and
          ecosystem service rewards. Collaborating with Environmental
          accountants, we combine their environmental metrics expertise with our
          impact investments, ensuring financial strategies prioritize returns
          while contributing tangibly to ecosystem sustainability. This
          transformative partnership promises a harmonious blend of financial
          success and ecological well-being, driving meaningful change beyond
...
export default AccountantsNetworks;
